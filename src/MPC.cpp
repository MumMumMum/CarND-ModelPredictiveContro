...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      // trajectory
      fg[0] += 12000*CppAD::pow(vars[cte_start + i] , 2);
      fg[0] += 9000*CppAD::pow(vars[epsi_start + i] , 2);
    ...
    for (int i = 0; i < N - 2; i++) {
      fg[0] += 9000*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += 1000*CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
      //fg[0] += 3000*CppAD::pow(vars[epsi_start + i + 1] - vars[epsi_start + i], 2);
      //fg[0] += 3000*CppAD::pow(vars[cte_start + i + 1] - vars[cte_start + i], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  for (int i = delta_start; i < a_start; i++) {
    ...
    //vars_lowerbound[i] = -M_PI/9; //20deg
    //vars_upperbound[i] =  M_PI/9;
